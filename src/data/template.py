...
from extras import ReasonFlux_logging as logging
...
class Template:
    ...
    def encode_multiturn(
        ...
        """
        encoded_messages = self._encode(tokenizer, messages, system, tools)#<|im_start|>system\nYou are a helpful assistant.<|im_end|>+instruction + input 转为token_id以及output 转为token_id
    ...
    def _encode(
        ...
        """
        system = system or self.default_system #self.default_system:You are a helpful assistant.
        encoded_messages = []
        for i, message in enumerate(messages):
            ...
            if i == 0:
                ...
                if system or tools:
                    ...
                    elements += self.format_system.apply(content=(system + tool_text)) #<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n

            if message["role"] == Role.USER.value:
                elements += self.format_user.apply(content=message["content"], idx=str(i // 2)) #添加'<|im_start|>user\n prompt + input <|im_end|>\n<|im_start|>assistant\n'
            elif message["role"] == Role.ASSISTANT.value:
                elements += self.format_assistant.apply(content=message["content"]) #output +<|im_end|>\n
            ...
            encoded_messages.append(self._convert_elements_to_ids(tokenizer, elements)) #将list(elements)中的句子转为token_id
